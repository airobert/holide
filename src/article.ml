...
let process_command cmd stack =
  if String.length cmd = 0 then stack else
    ...
    if Name.is_numerical c then process_num stack cmd else
      (* let () = Output.print_verbose "Debuging information: Processing %s\n%!" cmd in *)
      ...
      (* this is added for translation of MP, DISCH , SPEC and GEN*)
      | "mp", Thm(thm_p) :: Thm (thm_pq) :: stack -> Thm((Thm.mp thm_pq thm_p)) :: stack
      | "disch", Thm(thm_p) :: Term (x) :: stack -> Thm((Thm.disch x thm_p)) :: stack
      | "spec" , Thm(thm_p) :: Term (x) :: stack -> Thm( (Thm.spec x thm_p)) :: stack
      | "gen" , Thm(thm_p) :: Var (x) :: stack -> Thm( (Thm.gen x thm_p)) :: stack
