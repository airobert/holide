...
let translate_var tcontext x = 
  if not(List.mem x tcontext) then Name.hol "bool" (*I translate it too bool for example*)
  else 
  Name.escape x
...
let rec translate_type tcontext a =
  try
    let id = TypeSharing.find a in
    let fv = free_vars [] a in
    let id' = Dedukti.var (translate_type_id id) in
    let fv' = List.map Dedukti.var (List.map (translate_var tcontext) fv) in
    Dedukti.apps id' fv'
  with Not_found ->
    match a with
    | Var(x) ->
      Dedukti.var (translate_var tcontext x)
    | App(op, args) ->
      let op' = Dedukti.var (translate_op op) in
      let args' = List.map (translate_type tcontext) args in
      Dedukti.apps op' args'

(** Translate the list of type variables [x1; ...; xn]
    into the Dedukti terms [x1 : type; ...; xn : type] *)
let translate_vars tcontext vars =
  let tcontext = List.rev_append vars tcontext in 
  let vars' = List.map (fun x -> (translate_var tcontext x, translate_kind 0)) vars in 
  (vars', tcontext)
...
let define_type a =
  if not !Output.just_check && not (TypeSharing.mem a) then (
      let fv = free_vars [] a in
      let fv', tcontext = translate_vars [] fv in
      let arity' = translate_kind (List.length fv) in
      let a' = Dedukti.lams fv' (translate_type tcontext a) in
